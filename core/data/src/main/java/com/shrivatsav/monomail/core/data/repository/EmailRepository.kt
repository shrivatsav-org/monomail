...
class EmailRepository(
    ...
    suspend fun startBackgroundDeepSync(days: Int, accountId: String) {
        ...
        do {
            ...
            pageIndex++
            // time-based catchup for multi-page sync (prevents drops)
    ...
    @Volatile private var activeBackfillJob: kotlinx.coroutines.Job? = null
    ...
    suspend fun startBodyBackfill(accountId: String, maxEmails: Int = 500) {
        activeBackfillJob = kotlin.coroutines.coroutineContext[kotlinx.coroutines.Job]
        val provider = getProviderForAccount(accountId) ?: run {
    ...
    /**
     * Cooldown-guarded trigger used by the periodic sync worker: skips if a
     * sweep finished recently, starts the foreground service otherwise.
     */
    ...
    /** User-initiated stop from the notification's Cancel action: cancels the
     *  running sweep and removes its notification immediately. */
    fun cancelBodyBackfill() {
        ...
        // notifications (BodyBackfillService.startForeground posts 0xBB) — the
        // only way to remove it instantly is dropping the service's foreground
        // state, which stopping it does. No-op on the plain-coroutine path.
        try {
            context.stopService(android.content.Intent(context, com.shrivatsav.monomail.core.data.worker.BodyBackfillService::class.java))
        } catch (_: Exception) {}
    }
